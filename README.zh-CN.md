...
Note Database 可以把 Markdown 文件和 frontmatter 属性组织成可编辑的表格、看板、画廊、列表和图表视图。它保持本地优先，直接使用普通 Markdown 文件，并把数据库配置保存在你的 vault 中。

## 核心亮点

- **五种数据库视图**：同一组笔记可以在表格、看板、画廊、列表和图表之间切换。
...
| 图表 |
| --- |
| ![图表视图](assets/screenshots/zh-chart-view.png) |
| 将当前搜索与筛选结果聚合成可配置图表，支持汇总、明细钻取、色板和导出。 |
...
![图表设置](assets/screenshots/zh-chart-options.png)
