...
1.0.6 是一个 hotfix，主要修复数据安全与新用户首屏体验：

- 计算字段现在只会同步到匹配数据库来源边界的笔记，不再写入无关 Markdown 文件。
...
- **计算字段 / 公式**：公式编辑器支持字段引用、常用函数、语法高亮、实时结果、引用字段值和逐步计算预览；计算结果会在相关保存事件发生时同步回 frontmatter。
