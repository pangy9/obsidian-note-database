...
**完整的 .base 来源规则导入**：支持方法链（file.inFolder("X").and(file.hasTag("#Y"))）、AND/OR/NOT 逻辑、比较运算、类型检查、正则、前缀/后缀、空值/真值、取反、链接/标签/属性检查、多标签筛选，以及视图级筛选，全部自动转换。无法映射为结构化规则的复杂表达式会保留为可编辑的表达式规则。
**.base 公式导入保留原生语法**：导入的公式保留 Bases 语法，由专用引擎求值，支持方法链、条件判断、日期运算、时长计算、链接解析、字符串/数字/列表/对象辅助函数，以及汇总聚合（sum/count/average/unique/join）。跨公式依赖按定义顺序解析。
**计算字段同步模式**：每个数据库现在可以选择仅展示（虚拟值，不写入 frontmatter）、手动同步（点击同步按钮后写回）和自动同步（值变化时自动写回）。工具栏在手动模式下显示同步按钮，自动模式下显示实时指示器。
**更智能的来源规则编辑器**：字段选择器按笔记属性、公式字段和文件属性分组。操作符下拉菜单现在按字段类型过滤——数字字段不再显示文件夹操作符，标签字段不再显示前缀操作符等。已保存规则中的旧操作符会保留在兼容性分组中。
**Bug 修复**：数据安全、计算字段同步、视图状态持久化、撤销/重做可靠性、嵌入视图权限、CSV 导入导出、数据库删除、属性重命名迁移和 .base 转换准确性。
**安全与审核修复**：用安全的递归下降表达式解析器替代 `new Function`，用 DOMParser 替代 `createContextualFragment`，解决了 Obsidian 插件审核中的全部 Error 级问题。
**弹出窗口兼容性**：所有 `document`、`setTimeout`、`clearTimeout` 和 `requestAnimationFrame` 调用现在使用窗口作用域的等价形式（`window.activeDocument`、`window.setTimeout` 等），确保在 Obsidian 弹出窗口中正确运行。
