...
- 修复数据库文件 tab 和 header 行为，让 `db_view: true` 文件使用 Obsidian 原生文件视图路径 header。
- 嵌入数据库引用优先使用 database id，移动或重命名数据库文件后嵌入块更不容易失效。
- 数字字段新增评分、水平进度条、圆环进度条显示样式，并支持图标、emoji、颜色、最大值、除数和是否显示数值等配置。
- datetime 分组新增“忽略时刻”选项，可按日期聚合；修复 datetime 编辑 popover 的时刻输入分段控件。
- 新增每组最多显示记录数，并支持逐批展开或完全展开。
- 改进手机端设置页、状态预设下拉、表格 checkbox 对齐、卡片/列表字段宽度、列宽调整入口和 popover 输入框样式。
