...
- **属性冲突处理**：跨数据库文件检测同名 frontmatter 属性的底层存储形态冲突，在保存类型变更前给出修复入口。

![TODO：属性类型冲突处理弹窗](assets/screenshots/zh-property-type-conflict.png)

- **日历和时间线搜索结果**：搜索时显示事件结果、当前范围数量、点击跳转和跳转高亮。

![TODO：日历/时间线搜索结果浮层](assets/screenshots/zh-calendar-timeline-search-results.png)

- **浮动事件详情面板**：在日历/时间线中直接查看和编辑事件属性，不需要离开当前视图。

![TODO：浮动事件详情面板](assets/screenshots/zh-record-detail-panel.png)

- **表格编辑体验**：键盘导航、选项弹窗焦点、复制/克隆记录和 CSV/Markdown 粘贴更稳定。
- **字段管理打磨**：文件属性创建快捷入口、删除列时增加仅清理属性选项、日期/时间编辑选择器小日历，以及列表紧凑字段布局。
