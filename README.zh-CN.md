...
- **表格选择列对齐**：切换排序规则开关时，表头与各行的 checkbox 现在始终保持对齐。改用绝对定位固定 checkbox 位置，消除了由单元格边框引起的 1px 抖动。手机端选择列宽度也从 62px 收窄到 48px，腾出更多横向空间。
...
- 修复表格选择列 checkbox 对齐：表头与行的 checkbox 改用绝对定位固定，消除切换排序规则时的 1px 抖动（选择列的实际内容宽度因单元格边框比声明宽度少 1px）。
- 手机端选择列宽度从 62px 收窄到 48px。
