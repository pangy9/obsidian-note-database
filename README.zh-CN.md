...
## 1.0.4 更新亮点（相较于 1.0.3）

- **手机端全面适配**：工具栏、视图切换、属性面板、单元格编辑和公式编辑器现已支持在手机和平板上完整使用。
- **手机端拖拽替代方案**：属性列排序、排序规则、数据库列表、状态选项、单元格选项、分组顺序等纯拖拽列表新增上移/下移按钮，手机端隐藏拖拽手柄。
- **手机端表格选区**：Dashboard 与嵌入表格改为点选模式——点击设锚点，再点击扩展选区；行 checkbox 使用上次选中行作为范围锚点。
- **手机端卡片移动**：看板卡片新增触摸友好的移动菜单（置顶/置底、移到某卡片前后、移动到其他分组/子组），替代拖拽操作。
- **禁用不可靠的手机端拖拽**：列头拖拽排序、列宽拖拽、行分组跨组拖拽、单元格填充角标、看板列拖拽/列宽拖拽、画廊卡片宽度拖拽在手机端隐藏。
- **手机端工具栏**：搜索框移到视图 tab 行；工具按钮换行到第二行可横向滚动；视图 tab 禁用拖拽排序，改用菜单提供置顶/上移/下移/置底。
- **手机端公式编辑器**：禁用输入框内联联想弹窗，避免键盘遮挡；底部字段/函数面板保留搜索和点击插入功能。
- **文本编辑体验优化**：桌面端文本单元格编辑改为从单元格位置浮出的 contextual layer，不再是带标题栏和 Cancel/Save 按钮的 modal card。
- **自适应列宽算法改进**：自动列宽改用 `canvas.measureText()` 轻量测量，表头、普通文本和选项 badge 分别按对应字号/字重估算，结果更紧凑准确。
- **视觉打磨**：单元格选项弹窗改为 flat 风格、pill 圆角、更宽松的间距；搜索框聚焦不再出现高亮背景块；数据库描述编辑框自适应高度。
- **问题修复**：移动端浮层编辑器定位问题；属性编辑窗口元素错位；公式错误预览重复显示。
