...
const en: Dictionary = {
  ...
  "modal.migrateValuesDesc": "When checked: in each note, existing values will be overwritten and the old property name's frontmatter will be deleted. When unchecked: the database reads frontmatter values from note files as-is. In both cases, the property type in Obsidian will be updated to match the current column type.",
  ...
  "column.deletedColumn": "Deleted column, removed frontmatter \"{key}\" from {count} files",
...
const zhCN: Dictionary = {
  ...
  "modal.migrateValuesDesc": "勾选后：每条 .md 文件中，已有值会被覆盖，旧属性名对应的 frontmatter 会被删除。不勾选时：数据库直接读取笔记文件中的 frontmatter 值。无论是否勾选，Obsidian 中的属性类型都会同步更新为当前列的类型。",
  ...
  "column.deletedColumn": "已删除列，并从 {count} 个文件中移除 frontmatter「{key}」",
